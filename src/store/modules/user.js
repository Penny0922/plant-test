...
const mutations = {
  DO_LOGIN(state, payload) {
    ...
    state.token = payload.userInfo.data.data.token;
    state.userInfo = payload.userInfo.data.data;

    console.log("用户！！", state.userInfo);
    // 缓存本地，兼容H5刷新。默认8小时候过期
    /* cache.put('token', payload.userInfo.token, 60 * 60 * 8)
		cache.put('userInfo', payload.userInfo, 60 * 60 * 8) 	// 只包含部分用户信息 */
    sessionStorage["token"] = payload.userInfo.data.data.token;
    sessionStorage["userInfo"] = JSON.stringify(payload.userInfo.data.data);
  },
  /* DO_LOGOUT(state) {
